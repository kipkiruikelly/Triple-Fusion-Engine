...
class CompetitionModel(db.Model):
    ...
    initial_balance = db.Column(db.Float, default=10_000.0)
    status          = db.Column(db.String(12), default='upcoming')  # upcoming/active/completed
...
class CompetitionEntry(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint('competition_id', 'user_id', name='uq_comp_user'),)
...
class ModelVersion(db.Model):
    ...
    id            = db.Column(db.Integer, primary_key=True)
    ticker        = db.Column(db.String(12), nullable=False, index=True)
    model_type    = db.Column(db.String(20), nullable=False)  # lr, rf, xgb, lgb, lstm, stacking
    version       = db.Column(db.String(16), nullable=False)
    file_path     = db.Column(db.String(256), nullable=False)
    metrics_json  = db.Column(db.Text, nullable=True)
    feature_hash  = db.Column(db.String(64), nullable=True)
